...
export default function ProductDetail() {
  ...
  const discountPercent = hasDiscount
    ...
    : 0;
  const savedAmount = hasDiscount ? Number(product.price) - Number(product.discount_price) : 0;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-12 px-4 pb-16 pt-6 sm:px-6 lg:px-8">
      <div className="grid gap-8 md:grid-cols-2 md:gap-10 lg:gap-14">
        {/* Image */}
        <div className="group aspect-square overflow-hidden rounded-3xl border border-paper-line bg-white shadow-sm">
          {product.image_url ? (
            <img
        ...
        <div className="flex flex-col">
          {product.brand ? (
            <p className="text-xs font-semibold uppercase tracking-wider text-ink-muted">{product.brand}</p>
          ) : null}
          ...
          <div className="mt-6 rounded-2xl border border-paper-line bg-paper-raised/60 p-5">
            <div className="flex flex-wrap items-end gap-x-3 gap-y-1">
              <span className="font-display text-3xl font-bold text-ink sm:text-4xl">{formatPrice(price)}</span>
              {hasDiscount ? (
                <>
      ...
      <section>
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <h2 className="font-display text-2xl font-bold text-ink sm:text-3xl">Ratings &amp; Reviews</h2>
          {isAuthenticated && !myReview && !showReviewForm ? (
            <Button variant="outline" size="sm" icon={PenLine} onClick={() => setShowReviewForm(true)}>
              Write a review
            </Button>
          ) : null}
        </div>
                  ...
                  {breakdown.map(({ star, count, percent }) => (
                    <div key={star} className="flex items-center gap-3">
                      <span className="flex w-10 shrink-0 items-center gap-1 text-xs font-semibold text-ink-soft">
                        {star} <Star size={11} className="fill-gold-500 text-gold-500" />
                      </span>
            ...
            {reviewsError ? (
              ...
              <div className="relative overflow-hidden rounded-3xl border border-dashed border-gold-300/70 bg-gradient-to-b from-gold-50/70 to-paper-raised/40 px-6 py-14 text-center">
                <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-white shadow-sm ring-1 ring-gold-200">
                  <MessageSquareText size={24} className="text-gold-600" strokeWidth={1.75} />
                </div>
                <p className="mt-4 font-display text-lg font-bold text-ink">No reviews yet</p>
                <p className="mx-auto mt-1.5 max-w-sm text-sm leading-relaxed text-ink-muted">
                  Only customers who&apos;ve purchased this product can leave a review. Be the
                  first to share what you thought.
                </p>
                {!showReviewForm ? (
                  <Button
                    variant="outline"
