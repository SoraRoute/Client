...
export default function ForgotPassword() {
  ...
  async function submitEmail(values) {
    try {
      await axiosInstance.post(CUSTOMER.FORGOT_PASSWORD, { email: values.email });
  ...
  async function submitReset(values) {
    ...
    try {
      await axiosInstance.post(CUSTOMER.RESET_PASSWORD, {
        email,
        otp: values.otp,
  ...
  if (step === "reset") {
    return (
      <div className="mx-auto max-w-sm py-10">
        <h1 className="text-center font-display text-2xl font-semibold text-ink">Reset password</h1>
        <p className="mt-1 text-center text-sm text-ink-muted">Enter the OTP sent to {email}</p>

        <form onSubmit={resetForm.handleSubmit(submitReset)} className="mt-8 space-y-4">
          <Input
            label="OTP"
            inputMode="numeric"
  ...
  return (
    <div className="mx-auto max-w-sm py-10">
      <h1 className="text-center font-display text-2xl font-semibold text-ink">Forgot password</h1>
      <p className="mt-1 text-center text-sm text-ink-muted">
        We&apos;ll send a one-time code to your email
      </p>

      <form onSubmit={emailForm.handleSubmit(submitEmail)} className="mt-8 space-y-4">
        <Input
          label="Email"
          type="email"
