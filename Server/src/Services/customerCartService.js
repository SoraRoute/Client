...
class CustomerCartService{
    async addToCart(customerId,productId, quantity){
        ...
        if(!product){
         ...
         const cartItem=await customerCartRepository.findCartItem(cart.id, productId);
         if(cartItem){
            const rowsUpdated=await customerCartRepository.updateCartItemQuantity(cart.id, productId,cartItem.quantity+quantity);
            if(rowsUpdated===0){
         ...
         const cartItemId= await customerCartRepository.addProductToCart(cart.id, productId,quantity);
         return {
            success:true,
...
const cartItem=await customerCartRepository.findCartItem(cart.id, productId);
    ...
    const rowsDeleted=await customerCartRepository.removeProductFromCart(cart.id, productId);
    if(rowsDeleted===0){
