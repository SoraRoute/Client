...
class customerAddressController {
 ...
 async getAddresses(req,res){
    try {
        const result = await customerAddressService.getAddresses(req.customerId);
 ...
 async getAddress(req,res){
    try{
        const result=await customerAddressService.getAddress(req.params.id,req.customerId);
        return res.status(200).json(result);;
    }
    catch(error){
        return res.status(400).json({
